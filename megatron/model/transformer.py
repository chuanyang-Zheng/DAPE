...
class ParallelSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.pos_emb == "alibi":
            self.alibi_embed = AliBi(
                ...
                noise_max_length=neox_args.noise_seq_length,

            )
        if self.pos_emb == "alibi_c":
            self.alibi_c_embed = AliBi_C(
                neox_args.num_attention_heads,
                neox_args.model_parallel_size,
                mpu.get_model_parallel_rank(),
                noise_max_length=neox_args.noise_seq_length,
                mlp_width=neox_args.mlp_width
            )
        if self.pos_emb == "fire":
            self.fire = FIRE(neox_args.num_attention_heads, neox_args.mlp_width, 0.1,
                                        neox_args.noise_seq_length, noise_max_length=neox_args.noise_seq_length)
        if self.pos_emb == "fire_c":
            self.fire_c = FIRE_C(neox_args.num_attention_heads, neox_args.mlp_width, 0.1,
                                            neox_args.noise_seq_length, noise_max_length=neox_args.noise_seq_length)

        if self.pos_emb == "nope_c":
            self.nope_c = Nope_C(neox_args)
        if self.pos_emb == "nope_c_ffn":
            self.nope_c_ffn = Nope_C_FFN(neox_args)

        if self.pos_emb == "xl_bias":
            self.xl_bias = RelativePositionalEncodingLayer(neox_args.num_attention_heads,
                                                                self.hidden_size_per_attention_head,
                                                                neox_args.noise_seq_length,
                                                                mlp_width=neox_args.mlp_width,
                                                                scale_factor=(1.0 / self.norm_factor),
                                                                train_length=neox_args.seq_length)
        if self.pos_emb == "kerple":
            self.kerple = ParallelKerpleLog(neox_args)
        if self.pos_emb == "kerple_c":
            self.kerple_c = ParallelKerpleLog_C(neox_args, layer_number)


        if self.pos_emb == "rope_dapev1":
            self.dapev1 = DAPEV1(neox_args)

        if self.pos_emb == "rope_dapev2":
            self.dapev2 = DAPEV2(neox_args)

        if self.pos_emb == "alibi_dapev2":
            self.alibi_dapev2 = Alibi_DAPEV2(
                neox_args.num_attention_heads,
                neox_args.model_parallel_size,
                mpu.get_model_parallel_rank(),
                noise_max_length=neox_args.noise_seq_length,
        ...
        if self.pos_emb == "kerple_dapev2":
            self.kerple_dapev2 = Kerple_DAPEV2(neox_args, layer_number)
        if self.pos_emb == "fire_dapev2":
            self.fire_dapev2 = FIRE_DAPEV2(neox_args.num_attention_heads, neox_args.mlp_width, 0.1,
                                           neox_args.noise_seq_length, noise_max_length=neox_args.noise_seq_length, neox_args=neox_args)

        if self.pos_emb == "cope":
            self.cope = CoPE(64, self.hidden_size_per_attention_head)

        # TODO: this arg shouldn't need to be passed in - get from neox_args
        if self.pos_emb=="rotary_mlp" or self.pos_emb=="rope_dapev1" or self.pos_emb=="rope_dapev2":
            rotary=True
        if rotary:
    ...
    def attention(
        ...
        if self.pos_emb == "xl_bias":
            attention_scores=self.xl_bias(query_layer,key_layer)
        else:
        ...
        if self.pos_emb == "alibi_c":
            attention_scores = self.alibi_c_embed(attention_scores)
        if self.pos_emb == "fire":
            attention_scores = self.fire(attention_scores)
        if self.pos_emb == "fire_c":
            attention_scores = self.fire_c(attention_scores)

        if self.pos_emb == "kerple":
            attention_scores = self.kerple(attention_scores)
        if self.pos_emb == "kerple_c":
            attention_scores = self.kerple_c(attention_scores)

        if self.pos_emb == "nope_c":
            attention_scores = self.nope_c(attention_scores)
        if self.pos_emb == "nope_c_ffn":
            attention_scores = self.nope_c_ffn(attention_scores)

        if self.pos_emb == "rope_dapev1":
            attention_scores = self.dapev1(attention_scores)

        if self.pos_emb == "rope_dapev2":
            attention_scores = self.dapev2(attention_scores)

        if self.pos_emb == "alibi_dapev2":
            attention_scores = self.alibi_dapev2(attention_scores)
        if self.pos_emb == "kerple_dapev2":
            attention_scores = self.kerple_dapev2(attention_scores)
        if self.pos_emb == "fire_dapev2":
            attention_scores = self.fire_dapev2(attention_scores)

        if self.pos_emb == "cope":
            query_layer_this = query_layer.view(
                output_size[2], output_size[0], output_size[1], -1
            )
            query_layer_this = query_layer_this.permute(1, 2, 0, 3)
            # print(query_layer_this.shape)
            attention_scores = self.cope(query_layer_this, attention_scores)
