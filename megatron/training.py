...
def forward_step_eval_last(
    ...
    if neox_args.train_impl == "normal":
        ...
        if (
            ...
            loss_mask = loss_mask[:, -min(neox_args.eval_last, loss_mask.shape[1]):].contiguous() # the default value of eval_last is 256.
            labels = labels[:, -min(neox_args.eval_last, labels.shape[1]):].contiguous()
        main_loss = cross_entropy(
            outputs[:, -min(neox_args.eval_last, outputs.shape[1]):], (labels, loss_mask), _fp16=neox_args.fp16_lm_cross_entropy
        )
        if neox_args.moe_num_experts > 1:
            if neox_args.moe_type == "deepspeed":
                moe_loss = neox_args.moe_loss_coeff * sum(m.item() for m in moe_losses)
...
def train(
    ...
    while iteration < neox_args.train_iters:
        ...
        if (
            ...
            batch_size = [128, 64, 32, 16, 8, 1, 1, 1, 1, 1, 1, 1]

            ori_batch = neox_args.batch_size
            ori_length = neox_args.seq_length
            for index, valid_seq_length in enumerate([128, 256, 512, 1024, 2048, 4096, 8192]):
                if valid_seq_length < ori_length:
                    continue
                print("Seq Length & Batch {}".format(valid_seq_length, batch_size[index]))
                neox_args.batch_size = batch_size[index]
                neox_args.seq_length = valid_seq_length
                ...
                prefix = "Seq_Length {}: the end of training for val data".format(valid_seq_length)
                evaluate_and_print_results(
                    neox_args=neox_args,
                    prefix=prefix,
                    forward_step_func=forward_step_eval_last,
                    data_iterator=valid_data_iterator,
                    model=model,
                    iteration=iteration,
                    verbose=False,
            ...
            neox_args.batch_size = ori_batch
            neox_args.seq_length = ori_length
